...
def prj2rw(D, Kd, keep_organized=True, cutoff=5., scale_d=1.0000000474974513e-03):
    '''
    Algin some other modality to depth-floor-1. The slow more-readable version of this code whould be: align_to_depth_slow,
    which is commented underneath.
    :param D: depth-floor-1 frame
    ...
    if cutoff > 0.:
        cut_mask = z > cutoff
        if keep_organized:
            x[cut_mask] = y[cut_mask] = z[cut_mask] = np.nan
        else:
            x = x[~cut_mask]
            y = y[~cut_mask]
            z = z[~cut_mask]
...
def main():
    ...
    D_rw = prj2rw(D_prj, Kd, keep_organized=False)
