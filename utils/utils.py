...
def rescale_boxes(boxes, current_dim, original_shape):
    ...
    rescaled_boxes = boxes.clone().detach()
    rescaled_boxes[:, 0] = ((boxes[:, 0] - pad_x // 2) / unpad_w) * orig_w
    rescaled_boxes[:, 1] = ((boxes[:, 1] - pad_y // 2) / unpad_h) * orig_h
    rescaled_boxes[:, 2] = ((boxes[:, 2] - pad_x // 2) / unpad_w) * orig_w
    rescaled_boxes[:, 3] = ((boxes[:, 3] - pad_y // 2) / unpad_h) * orig_h
