...
def evaluate(backbone, model, data_configs, output, iou_thres, conf_thres, nms_thres, img_size, batch_size, num_workers=8):
    backbone.eval()
    model.eval()
    ...
    for batch_i, (batch_data, img_size) in enumerate(tqdm.tqdm(dataloader, desc="Detecting objects")):
        ...
        with torch.no_grad():
            outputs, _, maps = backbone(imgs, return_maps={'1': [21,28,11]})
            loss, _, outputs = model(maps['1'], targets, img_dim=416) # targets only evaluation loss
