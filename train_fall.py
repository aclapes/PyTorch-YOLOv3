...
from test_fall import evaluate
...
if __name__ == "__main__":
    ...
    parser.add_argument("--backbone_def", type=str, default="config/yolov3-1cls-tiny.cfg", help="path to model definition file")
    parser.add_argument("--model_def", type=str, default="config/fallnet-1.cfg", help="path to model definition file")
    parser.add_argument("--data_config", type=str, default="config/senior2bbb_depth-post_0_0.85.data", help="path to data config file")
    parser.add_argument("--backbone_weights", type=str, help="if specified starts from checkpoint model")
    ...
    # Initiate model
    backbone_model = Darknet(opt.backbone_def).to(device)
    backbone_model.apply(weights_init_normal)

    # If specified we start from checkpoint
    if opt.backbone_weights:
        chkpt = torch.load(opt.backbone_weights, map_location=device)  # load checkpoint
        backbone_model.load_state_dict(chkpt['model'])
    else:
    ...
    for epoch in range(st_epoch, st_epoch + opt.epochs):
        ...
        for batch_i, (batch_data, img_size) in pbar:
            ...
            with torch.no_grad():
                dets_tmp, det_inds, maps = backbone_model(imgs, return_maps={'1': [21,28,11]})
                # dets_tmp = non_max_suppression2(dets_tmp, det_inds, 0.5, 0.2)

            backbone_img_dim = max(imgs[0].shape[-2:])
            loss, loss_items, outputs = model(maps['1'], targets, img_dim=backbone_img_dim)
            loss.backward()
